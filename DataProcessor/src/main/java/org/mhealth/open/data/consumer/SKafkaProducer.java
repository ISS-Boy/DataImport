...
public class SKafkaProducer implements SProducer{
    ...
    public void produce2Dest(Allergies record) {
        ...
        ProducerRecord<String, SAllergie> message = new ProducerRecord<String, SAllergie>("synthea_test1",
    ...
    public void produce2Dest(Observations record) {
        ...
        ProducerRecord<String, SObservation> message = new ProducerRecord<String, SObservation>("synthea_test2",
                record.getUserId(), avroMsg);
    ...
    public void produce2Dest(Patients record) {
        ...
        ProducerRecord<String, SPatient> message = new ProducerRecord<String, SPatient>("synthea_test3",
                record.getUserId(), avroMsg);
    ...
    public void produce2Dest(SRecord record) {
        ...
        if(record instanceof CarePlans){
             ...
             message = new ProducerRecord<String, SLine>("synthea_test4",
                record.getUserId(), avroMsg);
        }

        if(record instanceof Conditions){
            message = new ProducerRecord<String, SLine>("synthea_test5",
                    record.getUserId(), avroMsg);
        }

        if(record instanceof Encounters){
            message = new ProducerRecord<String, SLine>("synthea_test6",
                    record.getUserId(), avroMsg);
        }

        if(record instanceof Immunizations){
            message = new ProducerRecord<String, SLine>("synthea_test7",
                    record.getUserId(), avroMsg);
        }

        if(record instanceof Medications){
            message = new ProducerRecord<String, SLine>("synthea_test8",
                    record.getUserId(), avroMsg);
        }

        if(record instanceof procedures){
            message = new ProducerRecord<String, SLine>("synthea_test9",
                    record.getUserId(), avroMsg);
        }
